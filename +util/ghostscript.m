...
%}
    ...
    try
        ...
        hg2_str = ''; if util.using_hg2, hg2_str = ' or Matlab R2014a'; end
...
function path_ = gs_path
    ...
    path_ = util.user_string('ghostscript');
...
function good = check_store_gs_path(path_)
    ...
    if ~util.user_string('ghostscript', path_)
        filename = fullfile(fileparts(which('util.user_string.m')), '.ignore', 'ghostscript.txt');
